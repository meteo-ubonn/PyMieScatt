...
def ContourIntersection(Qsca, Qabs, wavelength, diameter, Qback=None, n=None,
    ...
    if Qback is None:
        ...
        if incErrors:
            ...
            graphElements = {'Qsca': _c[0], 'Qabs': _c[1], 'Qback': _c[2],
                             # contours
                             'QscaErrFill': _c[4], 'QscaErrOutline1': _c[5],
                             'QscaErrOutline2': _c[6],
                             'QabsErrFill': _c[7], 'QabsErrOutline1': _c[8],
                             'QabsErrOutline2': _c[9],
                             'SolMark': _c[10], 'SolFill': _c[11],
                             # the circly thingies at each solutions
            ...
            graphElements = {'Qsca': _c[0], 'Qabs': _c[1], 'Qback': _c[2],
                             # contours
                             'SolFill': _c[3], 'SolMark': _c[4],
                             # the circly thingies at each solution
    ...
    return (solutionSet, forwardCalculations, solutionErrors, fig, ax,
            graphElements)


def ContourIntersection_SD(Bsca, Babs, wavelength, dp, ndp, n=None, k=None,
                           nMin=1, nMax=3, kMin=0.00001, kMax=1, Bback=None,
                           gridPoints=60, interpolationFactor=2, maxError=0.005,
                           fig=None, ax=None, axisOption=0):
    ...
    if Bback is not None:
        ...
        if backError is not None:
            backErrorLevels = np.array(
                [Bback + x for x in [-backError, backError]])

    if n is None:
        scaChart = ax.contour(_n, _k, BscaList, scaLevels, origin='lower',
                              linestyles='dashdot', linewidths=1.5,
                              colors=('red'))
        if scaError is not None:
            scaErrorLevels = np.array([Bsca + x for x in [-scaError, scaError]])
            ax.contourf(_n, _k, BscaList, scaErrorLevels, origin='lower',
                        colors=('red'), alpha=0.15)
            ax.contour(_n, _k, BscaList, scaErrorLevels, origin='lower',
                       linewidths=0.5, colors=('red'), alpha=0.5)
    else:
        if type(n) in [list, tuple, np.ndarray]:
            scaErrorLevels = [n[0] * (1 + x) for x in [-n[1], n[1]]]
    ...
    if k is None:
        absChart = ax.contour(_n, _k, BabsList, absLevels, origin='lower',
                              linewidths=1.5, colors=('blue'))
        if absError is not None:
            absErrorLevels = np.array([Babs + x for x in [-absError, absError]])
            ax.contourf(_n, _k, BabsList, absErrorLevels, origin='lower',
                        colors=('blue'), alpha=0.15)
            ax.contour(_n, _k, BabsList, absErrorLevels, origin='lower',
                       linewidths=0.5, colors=('blue'), alpha=0.5)
    else:
        if type(k) in [list, tuple, np.ndarray]:
            absErrorLevels = [k[0] * (1 + x) for x in [-k[1], k[1]]]
    ...
    if Bback is not None:
        backChart = ax.contour(_n, _k, BbackList, backLevels, origin='lower',
                               linestyles='dotted', linewidths=1.5,
                               colors=('green'))
        if backError is not None:
            backErrorLevels = np.array(
                [Bback + x for x in [-backError, backError]])
            ax.contourf(_n, _k, BbackList, backErrorLevels, origin='lower',
                        colors=('green'), alpha=0.15)
            ax.contour(_n, _k, BbackList, backErrorLevels, origin='lower',
                       linewidths=0.5, colors=('green'), alpha=0.5)
    ...
    if Bback is None:
        if incErrors:
            ...
            graphElements = {'Bsca': _c[0], 'Babs': _c[1],  # contours
                             'Labels': labels,
                             'BscaErrFill': _c[2], 'BscaErrOutline1': _c[3],
                             'BscaErrOutline2': _c[4],
                             'BabsErrFill': _c[5], 'BabsErrOutline1': _c[6],
        ...
        if incErrors:
            ...
            graphElements = {'Bsca': _c[0], 'Babs': _c[1], 'Bback': _c[2],
                             # contours
                             'Labels': labels,
                             'BscaErrFill': _c[4], 'BscaErrOutline1': _c[5],
                             'BscaErrOutline2': _c[6],
                             'BabsErrFill': _c[7], 'BabsErrOutline1': _c[8],
...
def SurveyIteration(Qsca, Qabs, wavelength, diameter, tolerance=0.0005):
    ...
    error = lambda measured, calculated: np.abs(
        (calculated - measured) / measured)
    initial_m = Inversion(Qsca, Qabs, wavelength, diameter,
                          scatteringPrecision=0.015, absorptionPrecision=0.015,
                          spaceSize=85, interp=2)
    print(len(initial_m))
    ...
    for m in list(initial_m):
        ...
        for f, e in zip(factors, errors):
            ...
            while scaError > e:
                ...
                m = m + nStep
                trialQsca, trialQabs, trialQback = fastMieQ(m, wavelength,
                                                            diameter)
...
def SurveyIteration_SD(Bsca, Babs, wavelength, dp, ndp, tolerance=0.0005):
    ...
    error = lambda measured, calculated: np.abs(
        (calculated - measured) / measured)
    initial_m = Inversion_SD(Bsca, Babs, wavelength, dp, ndp,
                             scatteringPrecision=0.08, absorptionPrecision=0.08,
                             spaceSize=40, interp=2)
