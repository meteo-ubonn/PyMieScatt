...
def MieQ(m, wavelength, diameter, nMedium=1.0, asDict=False,
    ...
    if x == 0:
        ...
        g = (4 / (qsca * x2)) * np.sum((n2 * (
                    an.real * g1[0] + an.imag * g1[1] +
...
def RayleighMieQ(m, wavelength, diameter, nMedium=1.0, asDict=False,
                 asCrossSection=False):
...
def AutoMieQ(m, wavelength, diameter, nMedium=1.0, crossover=0.01, asDict=False,
             asCrossSection=False):
    ...
    if x == 0:
        ...
        return RayleighMieQ(m, wavelength, diameter, nMedium, asDict=asDict,
                            asCrossSection=asCrossSection)
    else:
        return MieQ(m, wavelength, diameter, nMedium, asDict=asDict,
                    asCrossSection=asCrossSection)


def LowFrequencyMieQ(m, wavelength, diameter, nMedium=1.0, asDict=False,
                     asCrossSection=False):
...
def LowFrequencyMie_ab(m, x):
    ...
    a1 = (-2j * x3 / 3) * LL - (2j * x5 / 5) * LL * (m2 - 2) / (m2 + 2) + (
                4 * x6 / 9) * (LL ** 2)
...
def Mie_SD(m, wavelength, dp, ndp, nMedium=1.0, interpolate=False,
           asDict=False):
    ...
    for i in range(_length):
        Q_ext[i], Q_sca[i], Q_abs[i], g[i], Q_pr[i], Q_back[i], Q_ratio[
            i] = AutoMieQ(m, wavelength, dp[i], nMedium)
    ...
    if asDict:
        return dict(Bext=Bext, Bsca=Bsca, Babs=Babs, G=bigG, Bpr=Bpr,
                    Bback=Bback, Bratio=Bratio)
    else:
        return Bext, Bsca, Babs, bigG, Bpr, Bback, Bratio


def ScatteringFunction(m, wavelength, diameter, nMedium=1.0, minAngle=0,
                       maxAngle=180, angularResolution=0.5, space='theta',
    ...
    if space in ['q', 'qspace', 'QSPACE', 'qSpace']:
        ...
        if minAngle == 0:
            minAngle = 1e-5
        measure = np.logspace(np.log10(minAngle), np.log10(maxAngle),
                              _steps) * np.pi / 180
        _q = True
    else:
    ...
    if normalization in ['m', 'M', 'max', 'MAX']:
        ...
        SU /= trapz(SU, measure)
    if _q:
        measure = (4 * np.pi / wavelength) * np.sin(measure / 2) * (
                    diameter / 2)
    return measure, SL, SR, SU


def SF_SD(m, wavelength, dp, ndp, nMedium=1.0, minAngle=0, maxAngle=180,
          angularResolution=0.5, space='theta', angleMeasure='radians',
          normalization=None):
...
def MieQ_withDiameterRange(m, wavelength, nMedium=1.0, diameterRange=(10, 1000),
                           nd=1000, logD=False):
    ...
    wavelength /= nMedium
    if logD:
        diameters = np.logspace(np.log10(diameterRange[0]),
                                np.log10(diameterRange[1]), nd)
    else:
...
def MieQ_withWavelengthRange(m, diameter, nMedium=1.0,
                             wavelengthRange=(100, 1600), nw=1000, logW=False):
    ...
    if type(_m) == complex and len(_wavelengthRange) == 2:
        if logW:
            wavelengths = np.logspace(np.log10(_wavelengthRange[0]),
                                      np.log10(_wavelengthRange[1]), nw)
        else:
            wavelengths = np.linspace(_wavelengthRange[0], _wavelengthRange[1],
                                      nw)
        _qD = [AutoMieQ(_m, wavelength, diameter) for wavelength in wavelengths]
    elif type(_m) in [np.ndarray, list, tuple] and len(_wavelengthRange) == len(
            _m):
        wavelengths = _wavelengthRange
        _qD = [MieQ(emm, wavelength, diameter) for emm, wavelength in
               zip(_m, wavelengths)]
    else:
        warnings.warn(
            "Error: the size of the input data is mismatched. "
            "Please examine your inputs and try again.")
        return
...
def MieQ_withSizeParameterRange(m, nMedium=1.0, xRange=(1, 10), nx=1000,
                                logX=False):
    ...
    if logX:
        xValues = list(
            np.logspace(np.log10(_xRange[0]), np.log10(_xRange[1]), nx))
    else:
...
def Mie_Lognormal(m, wavelength, geoStdDev, geoMean, numberOfParticles,
                  nMedium=1.0, numberOfBins=10000, lower=1, upper=1000,
                  gamma=[1], returnDistribution=False,
                  decomposeMultimodal=False, asDict=False):
    #  http://pymiescatt.readthedocs.io/en/latest/forward.html#Mie_Lognormal
    nMedium = nMedium.real
    m /= nMedium
    wavelength /= nMedium
    ithPart = lambda gammai, dp, dpgi, sigmagi: (gammai / (
                np.sqrt(2 * np.pi) * np.log(sigmagi) * dp)) * np.exp(
        -(np.log(dp) - np.log(dpgi)) ** 2 / (2 * np.log(sigmagi) ** 2))
    dp = np.logspace(np.log10(lower), np.log10(upper), numberOfBins)
    if all([type(x) in [list, tuple, np.ndarray] for x in
            [geoStdDev, geoMean]]):
        # multimodal
        if len(gamma) == 1 and (len(geoStdDev) == len(geoMean) > 1):
            ...
            ndpi = [numberOfParticles * ithPart(g, dp, dpg, sg) for g, dpg, sg
                    in zip(gamma, geoMean, geoStdDev)]
    ...
    if ndp[-1] > np.max(ndp) / 100 or ndp[0] > np.max(ndp) / 100:
        warnings.warn(
            "Warning: distribution may not be compact on the specified "
            "interval. Consider using a higher upper bound.")
    Bext, Bsca, Babs, bigG, Bpr, Bback, Bratio = Mie_SD(m, wavelength, dp, ndp)
    if returnDistribution:
        if decomposeMultimodal:
            if asDict:
                return dict(Bext=Bext, Bsca=Bsca, Babs=Babs, bigG=bigG, Bpr=Bpr,
                            Bback=Bback, Bratio=Bratio), dp, ndp, ndpi
            else:
                return Bext, Bsca, Babs, bigG, Bpr, Bback, Bratio, dp, ndp, ndpi
        else:
            if asDict:
                return dict(Bext=Bext, Bsca=Bsca, Babs=Babs, bigG=bigG, Bpr=Bpr,
                            Bback=Bback, Bratio=Bratio), dp, ndp
            else:
